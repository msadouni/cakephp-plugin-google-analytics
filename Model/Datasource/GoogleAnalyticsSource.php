...
class GoogleAnalyticsSource extends DataSource
{
    ...
    function account_data($queryData)
    {
        ...
        $data = array(array('Account' => array(
            'tableId' => $feed['dxp:dataSource']['dxp:tableId'],
            'name' => $feed['dxp:dataSource']['dxp:tableName'],
            'totalResults' => $feed['openSearch:totalResults'],
            'startIndex' => $feed['openSearch:startIndex'],
            'itemsPerPage' => $feed['openSearch:itemsPerPage'],
            'startDate' => $feed['dxp:startDate'],
            'endDate' => $feed['dxp:endDate'],
    ...
    function __dataPoints($feed)
    {
        ...
        foreach ($feed['entry'] as $key => $val)
        {
            ...
            if (is_numeric($key))
            {
                if (!empty($val['dxp:dimension'])) $dimension = $val['dxp:dimension'];
                if (!empty($val['dxp:metric'])) $metric = $val['dxp:metric'];
